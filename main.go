...
func (t Training) meanSpeed() float64 {
	if t.Duration != 0 {
		return (t.Action) * t.LenStep / MInKm / float64(t.Duration)
	}
	return 0
}
...
func (w Walking) Calories() float64 {
	SpeedInSquare := math.Pow((w.meanSpeed() * KmHInMsec), 2)
	return (CaloriesWeightMultiplier*w.Weight + (SpeedInSquare/w.Height/CmInM)*CaloriesSpeedHeightMultiplier*w.Weight) * float64(w.Duration) * MinInHours
}
